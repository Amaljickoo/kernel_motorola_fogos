...
static void a6xx_hwsched_touch_wakeup(struct adreno_device *adreno_dev)
{
	 ...
	 * has been completed.
	 */
	if (unlikely(test_bit(GMU_PRIV_PM_SUSPEND, &gmu->flags) ||
		!test_bit(GMU_PRIV_FIRST_BOOT_DONE, &gmu->flags)))
		return;

	if (likely(test_bit(GMU_PRIV_GPU_STARTED, &gmu->flags)))
		goto done;
...
static int a6xx_hwsched_power_off(struct adreno_device *adreno_dev)
{
	...
	if (unlikely(!test_bit(GMU_PRIV_GPU_STARTED, &gmu->flags)))
		return 0;
